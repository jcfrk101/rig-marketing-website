...
- The context's currentQuestion field tells you which slot the driver is answering — bind bare answers to it (a lone "car" when currentQuestion=vehicle_class → vehicle_class car; a lone "no" when currentQuestion=tire_spare → has_spare false). LEAN ON-TOPIC: a terse or typo'd message that plausibly answers the current question ("Hose" for the problem, "im on the sholder" for safety/location, "wheel bearing repacking") is on_topic with the answer bound — reserve off_topic for clearly unrelated content.
